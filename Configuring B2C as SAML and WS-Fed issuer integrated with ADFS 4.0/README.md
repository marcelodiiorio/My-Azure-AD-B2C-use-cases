This scenario allows you to use Azure AD B2C as SAML/WS-Fed tokens issuer. This is a consolidated scenario where we merge both SAML and WS-Fed token issuers.
...
Set-AdfsRelyingPartyTrust -TargetName "Name of your test app" -ClaimsProviderName @("Active Directory","B2C SAML2", "B2C WS-Fed")

Where B2C SAML2/WS-Fed is the name of the Claims Provider Trusts you have previously added. If you want to add more claims providers, just add ", <CPT name>" to the command above. If you don't append to existing content, you will remove all the CPTs, leaving only the new one.
